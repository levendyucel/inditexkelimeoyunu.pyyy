...
def get_options(correct_word, all_words):
    options = random.sample(all_words, 4)  # 4 rastgele kelime seç
    if correct_word not in options:
        options[random.randint(0, 3)] = correct_word
    options.append(correct_word)  # Toplam 5 seçenek olmalı
    random.shuffle(options)
    return options

# Başlık
st.title("Kelime Tahmin Oyunu")

# Kelimeler ve tanımlar listesini hazırlama
kelimeler = list(sozluk.keys())
kelime, tanim = random.choice(list(sozluk.items()))

# Seçenekleri oluşturma
options = get_options(kelime, kelimeler)

# Kullanıcıya tanım göster
st.write(f"Tanım: {tanim}")

# Kullanıcıdan seçenekleri seçmesi isteniyor
user_choice = st.radio("Bu tanım hangi kelimeye ait?", options)

# Cevabı göster butonu
show_answer = st.button("Cevabı Göster")

# Sonucu kontrol et
if show_answer:
    if user_choice == kelime:
        st.success("Tebrikler! Doğru cevap!")
        st.session_state.score += 1
    else:
        st.error(f"Yanlış cevap! Doğru cevap: {kelime}")

    st.session_state.questions_answered += 1

# Skoru göster
st.write(f"Skorunuz: {st.session_state.score}")
st.write(f"Toplam Sorular: {st.session_state.questions_answered}")

# Tekrar oynamak için buton
